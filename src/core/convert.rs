...
pub fn rgb8_to_gray32(rgb: &[u8]) -> Vec<f32> {
...
pub fn u16_to_u8(bytes: &[u8]) -> Vec<u8> {
...
// pub fn f32_to_u8(bytes: &[f32]) -> Vec<u8> {
...
pub fn rgb2arrayf32(img: RgbImage) -> Array3<f32> {
...
pub fn luma2arrayf32(img: GrayImage) -> Array2<f32> {
    let (width, height) = img.dimensions();
    let input = img.into_raw();
    let input_f32 = u8_to_f32(&input);
...
pub fn luma2array(img: GrayImage) -> Array2<u8> {
    let (width, height) = img.dimensions();
    let input = img.into_raw();
    Array2::from_shape_vec((height as usize, width as usize), input).unwrap()
}

pub fn rgb2array(img: RgbImage) -> Array3<u8> {
    let (width, height) = img.dimensions();
    let input = img.into_raw();
